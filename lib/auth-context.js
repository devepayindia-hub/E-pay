...
export function AuthProvider({ children }) {
    ...
    try {
      ...
      if (stored) {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          // Ensure all baseline SEED_USERS exist in local storage without wiping newly created users
          const existingEmails = new Set(parsed.map(u => u.email ? u.email.toLowerCase() : ''));
          const missingSeeds = SEED_USERS.filter(su => su.email && !existingEmails.has(su.email.toLowerCase()));
          if (missingSeeds.length > 0) {
            const combined = [...parsed, ...missingSeeds];
            localStorage.setItem('epay_users_registry', JSON.stringify(combined));
            return combined;
          }
          return parsed;
        }
      }
      localStorage.setItem('epay_users_registry', JSON.stringify(SEED_USERS));
      return SEED_USERS;
    } catch (e) {
      return SEED_USERS;
    }
  ...
  // Real-time Firestore snapshot listener for users collection - ONLY when authenticated via Firebase
  useEffect(() => {
    if (!db || typeof window === 'undefined' || !user || !auth?.currentUser) return;
  ...
  const login = async (email, password) => {
    ...
    try {
      ...
      if (!authenticatedProfile) {
        const registry = getLocalUserRegistry();
        let matchedUser = registry.find(u => u.email && u.email.toLowerCase() === cleanEmail);
        if (matchedUser) {
      ...
      // Verify password strictly against registered user passwordHash or password
      const storedPassword = matchedUser.passwordHash || matchedUser.password;
    ...
    if (db && typeof window !== 'undefined' && auth?.currentUser) {
      (async () => {
        try {
          const payloadToSave = {
            ...userData,
  ...
  const getAllUsers = async () => {
    let users = getLocalUserRegistry();
    if (db && typeof window !== 'undefined' && auth?.currentUser) {
      try {
        ...
        try {
          ...
          const topSnap = await getDocs(topUsersCol);
          if (!topSnap.empty) {
            topSnap.forEach(d => firestoreUsers.push({ uid: d.id, ...d.data() }));
          }
        } catch (_e) {}

        try {
          const tenantUsersCol = collection(db, 'tenants', 'default', 'users');
          const tenantSnap = await getDocs(tenantUsersCol);
          if (!tenantSnap.empty) {
            tenantSnap.forEach(d => firestoreUsers.push({ uid: d.id, ...d.data() }));
          }
        } catch (_e) {}
          ...
          firestoreUsers.forEach(u => {
            if (u.email) {
              ...
              emailMap.set(cleanE, {
                ...existing,
                ...u,
                passwordHash: existing.passwordHash || u.passwordHash || existing.password || u.password
              });
            }
          });
          users = Array.from(emailMap.values());
          saveLocalUserRegistry(users);
        }
      } catch (_e) {}
    }
    return users;
  ...
  const logout = async () => {
    setIsLoading(true);
    try {
      if (user && activeSession && db && typeof window !== 'undefined' && auth?.currentUser) {
