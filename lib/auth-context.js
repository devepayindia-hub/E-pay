...
const findUserInRegistry = (registry, searchEmail) => {
  ...
  return registry.find(u => {
    ...
    // Prefix match for @epay.in domain variants
...
export function AuthProvider({ children }) {
  ...
  const getLocalUserRegistry = useCallback(() => {
    ...
    try {
      ...
      // Collect all existing registered emails & aliases
      const existingEmailSet = new Set();
      registry.forEach(u => {
        if (u && u.email) existingEmailSet.add(u.email.trim().toLowerCase());
        if (u && Array.isArray(u.aliases)) {
          u.aliases.forEach(a => (a ? existingEmailSet.add(a.trim().toLowerCase()) : null));
        }
      });

      // Synchronize missing seed users and email aliases into the local registry
      let modified = false;
      SEED_USERS.forEach(su => {
        const suEmail = su.email ? su.email.trim().toLowerCase() : '';
        if (suEmail && !existingEmailSet.has(suEmail)) {
          registry.push(su);
          existingEmailSet.add(suEmail);
          if (Array.isArray(su.aliases)) {
            su.aliases.forEach(a => (a ? existingEmailSet.add(a.trim().toLowerCase()) : null));
          }
          modified = true;
        }
      });
